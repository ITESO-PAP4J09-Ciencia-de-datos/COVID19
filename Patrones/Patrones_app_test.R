...
Patrones <- read_excel("INDICADORES_MACROECONOMICOS/BD_INDICADORES_MACROECONOMICOS.xlsx", sheet = "Patrones")
...
server <- function(input, output, session) {
  ...
  output$plot2 <- renderPlot({ 
    ...
    Patrones_fore %>% autoplot(Patrones_tsbl1)
  })
...
output$plot2 <- renderPlot({ 
  Patrones_tsbl1 <- Patrones_tsbl %>%filter(Entidad == input$Entidad, Agrupacion == input$Agrupacion, cve2 == input$cve2)
  Patrones_tsbl_fore <- Patrones_tsbl1 %>%
    model(arima=ARIMA(Trabajadores_Asegurados),ets=ETS(Trabajadores_Asegurados)) %>%
    forecast(h=12)
  
  Patrones_tsbl_fore %>% autoplot(Patrones_tsbl,level=NULL)
})
